...
## Motora Product Lock (2026-08-06)

- The approved default is **Option 1 — Orange Light**. It is mobile-first and must remain the visual foundation: orange accent, warm cream canvas, airy cards, bright vehicle imagery, logo/brand discovery, and the existing header/footer/card/navigation anatomy. Add features without redesigning this UI unless the user explicitly asks.
- The approved alternate is **Option 3 — Gold Black** dark mode. A theme change must update surfaces, text, icons, controls, and accents together, using a readable black/charcoal base with a gold accent.
- Motora is one Indian marketplace across cars, motorcycles, scooters (including electric), commercial vehicles (including three-wheelers), bicycles, e-bikes, and kids' cycles. The home menu stays simple, with subcategories revealed on tap/hover where appropriate.
- Seller types are dealer, private seller, and collector. Each gets a shareable storefront/profile. Vehicle listings need media, key specifications, price, condition/registration disclosures, description, seller contact, and a path to the seller storefront.
- Premium listings use the future **Motora Checked** inspection/certification program. Stage 1 may model the UI only; verification workflow belongs to Stage 2.
- Stage 1 is a working frontend prototype with fixture data, responsive interaction, filtering, vehicle detail, dealer storefront, contact actions, theme switching, and browser-back-to-home behavior. Authentication, real uploads (photo/video/voice), admin catalogue operations, Instagram import, payments, API/database, native Android double-back exit, and model-data ingestion are Stage 2.
- Treat vehicle imagery and brand marks responsibly. Do not copy competitor listing photos or trademarks without rights. Production vehicle imagery must come from official/licensed sources or seller uploads; AI imagery can only be an explicitly labelled generic/color-preview image and must never represent an invented exact model.
- Responsive behavior is locked: narrow phones remain one-column and touch-first; phone widths must not assume an iPhone-shaped bezel. At 740px and above, render Motora as a fluid web/tablet workspace without device chrome, with a fixed left navigation rail and content that adapts through tablet and desktop widths.
